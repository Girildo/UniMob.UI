...
namespace UniMob.UI
{
        ...
        where TWidget : Widget
    {
        ...
        [MustUseReturnValue]
        protected RenderChildBuilder<TChildWidget> RenderChildT<TChildWidget>(
            WidgetBuilder<TChildWidget> builder,
            [CallerMemberName] string cacheKey = ""
        )
        ...
        protected readonly ref struct RenderChildBuilder<TChildWidget>
            where TChildWidget : Widget
        {
            ...
            public RenderChildBuilder(
                ViewState<TWidget> owner,
                string cacheKey,
                WidgetBuilder<TChildWidget> builder
            )
            {
            ...
            public TChildState As<TChildState>()
        ...
        private StateHolder CreateChildCached<TChildWidget>(
            string cacheKey,
            WidgetBuilder<TChildWidget> builder
        )
            where TChildWidget : Widget
        {
            ...
            {
                _renderChildCache[cacheKey] = cachedStateHolder = Create<TChildWidget, IState>(
                    StateLifetime,
        ...
        protected StateHolder<TChildState> CreateChild<TChildWidget, TChildState>(
            WidgetBuilder<TChildWidget> builder
        )
            where TChildWidget : Widget
            where TChildState : ViewState<TChildWidget>
        {
            return Create<TChildWidget, TChildState>(
                StateLifetime,
                new BuildContext(this, Context),
                builder
            );
